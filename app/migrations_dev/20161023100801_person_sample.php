...
class PersonSample extends AbstractMigration
{
    ...
    {
        $this->execute( <<<SQL
            INSERT INTO "person"."person" (id_person_person, lastname, firstname, phone, address, email, latlong, roles, church_id) VALUES
                ('66de0f5f-8e5c-497a-8643-b94393c8a899','Paul','Less', '{}', '1 rue du calvaire\n44000 Nantes', 'paul@example.com', '( 47.215767 , -1.5613887 )','{}', '1d481d08-744e-48c8-92ea-375a652449eb'),
                ('1e018920-dbf3-42d9-8e69-fb79be6689c9','Jean','Victor', '{"01 23 45 67 89"}', '1 place Graslin\n44000 Nantes', 'jeanjean@example.com', '( 47.2131678 , -1.5639745 )','{"ROLE_ADMIN"}', '1d481d08-744e-48c8-92ea-375a652449eb');
SQL
        );
        $this->execute( <<<SQL
            INSERT INTO "person"."person_church" (person_id, church_id) VALUES
...
SQL
        ...
        $this->execute(<<<SQL
          DELETE FROM "person"."person_church" WHERE person_id in (
